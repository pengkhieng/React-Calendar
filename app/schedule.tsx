...
const ScheduleItem: React.FC<ScheduleItemProps> = ({ time, title, link }) => (
        ...
        {currentDate && (
          <View style={styles.selectedDateContainer}>
            ...
            <ScrollView style={styles.eventScroll}>
              ...
              {/* Show Empty if both All-day and Event List are empty */}
              {!SAMPLE_EVENTS.some(
                (event) =>
                  event.start.split(" ")[0] === formatDate(currentDate) &&
                  event.isEvent
                ...
                !SAMPLE_EVENTS.some(
                  (event) =>
                    event.start.split(" ")[0] === formatDate(currentDate) &&
                    !event.isEvent
                ) && <Text style={styles.emptyText}>No event here</Text>}
            </ScrollView>
          </View>
        )}
      </View>
            ...
            return (
              ...
              >
                <View style={styles.weekDayHeader}>
                  <Text style={styles.weekDayLabel}>{WEEK_DAY_NAMES[i]}</Text>
                  <View
                    style={[
          ...
          {weekDays.map((day, i) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => setCurrentDate(new Date(day))}
              >
                <Text style={styles.dayHeaderLabel}>{WEEK_DAY_NAMES[i]}</Text>
                <View
                  style={[
...
const styles = StyleSheet.create({
  ...
  // Legacy styles (for ScheduleItem, potentially unused)
  list: {
    padding: 10,
  },
  itemContainer: {
